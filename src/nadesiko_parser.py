...
class EnhancedNadesikoParser:
    ...
    def parse_line(self, line):
        ...
        # データベース操作
        if self._is_database_operation(line):
            return self._parse_database_operation(line)
        
        # グラフ描画
        if self._is_plotting_operation(line):
            return self._parse_plotting_operation(line)
        
        # 画像処理
        if self._is_image_operation(line):
            return self._parse_image_operation(line)
        
        # 圧縮・解凍
        if self._is_compression_operation(line):
            return self._parse_compression_operation(line)
        
        # Excel操作
        if self._is_excel_operation(line):
            return self._parse_excel_operation(line)
        
        # メール送信
        if self._is_email_operation(line):
            return self._parse_email_operation(line)
        
        # スレッド処理
        if self._is_threading_operation(line):
            return self._parse_threading_operation(line)
        
        # タイマー・定期実行
        if self._is_timer_operation(line):
            return self._parse_timer_operation(line)
        
        # サブプロセス
        if self._is_subprocess_operation(line):
            return self._parse_subprocess_operation(line)
    ...
    def _is_database_operation(self, line):
        return any(word in line for word in ['データベース', 'DB', 'SQLite', 'SQL', 'テーブル', 'クエリ', 'SELECT', 'INSERT'])
    
    def _parse_database_operation(self, line):
        if '接続' in line or '開く' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                db_path = match.group(1)
                return f'sqlite3.connect("{db_path}")'
        if 'クエリ' in line or '実行' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                query = match.group(1)
        ...
        if 'テーブル作成' in line:
            ...
            if match:
                ...
                return f'cursor.execute("CREATE TABLE IF NOT EXISTS {table_def}")'
        return '# データベース操作'
    
    # ========== グラフ描画 ==========
    
    def _is_plotting_operation(self, line):
        return any(word in line for word in ['グラフ', 'プロット', 'matplotlib', 'チャート', '折れ線', '棒グラフ', '散布図'])
    
    def _parse_plotting_operation(self, line):
        self.imports.add('matplotlib.pyplot as plt')
        if '折れ線' in line:
            return 'plt.plot(x, y)'
        if '棒グラフ' in line:
            return 'plt.bar(x, y)'
    ...
    def _is_image_operation(self, line):
        return any(word in line for word in ['画像', 'イメージ', 'PIL', 'リサイズ', '回転', '切り抜き', 'フィルタ'])
    
    def _parse_image_operation(self, line):
        if '開く' in line or '読む' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                filepath = match.group(1)
                return f'Image.open("{filepath}")'
        if '保存' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                filepath = match.group(1)
                return f'image.save("{filepath}")'
        if 'リサイズ' in line:
            match = re.search(r'(\d+)x(\d+)', line)
            if match:
                w, h = match.groups()
                return f'image.resize(({w}, {h}))'
        if '回転' in line:
            match = re.search(r'(\d+)', line)
            if match:
                angle = match.group(1)
                return f'image.rotate({angle})'
        return '# 画像処理'
    
    # ========== 圧縮・解凍 ==========
    
    def _is_compression_operation(self, line):
        return any(word in line for word in ['圧縮', '解凍', 'zip', 'ZIP', 'アーカイブ'])
    
    def _parse_compression_operation(self, line):
        if '圧縮' in line:
            match = re.findall(r'「(.+?)」', line)
            if len(match) >= 2:
                files = match[0]
                zipname = match[1]
                return f'zipfile.ZipFile("{zipname}", "w").write("{files}")'
        if '解凍' in line:
            match = re.findall(r'「(.+?)」', line)
            if len(match) >= 2:
                zipname = match[0]
                dest = match[1]
                return f'zipfile.ZipFile("{zipname}", "r").extractall("{dest}")'
        return '# 圧縮・解凍'
    
    # ========== Excel操作 ==========
    
    def _is_excel_operation(self, line):
        return any(word in line for word in ['Excel', 'エクセル', 'シート', 'セル', 'ワークブック', 'xlsx'])
    
    def _parse_excel_operation(self, line):
        if '読む' in line or '開く' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                filepath = match.group(1)
                return f'openpyxl.load_workbook("{filepath}")'
        if '作成' in line:
            return 'openpyxl.Workbook()'
        if 'セル' in line:
            match = re.search(r'([A-Z]+\d+)に(.+?)を', line)
            if match:
                cell = match.group(1)
                value = match.group(2)
                return f'ws["{cell}"] = {value}'
        if '保存' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                filepath = match.group(1)
                return f'wb.save("{filepath}")'
        return '# Excel操作'
    
    # ========== メール送信 ==========
    
    def _is_email_operation(self, line):
        return any(word in line for word in ['メール', 'メール送信', 'メールする', 'SMTP', 'Eメール'])
    
    def _parse_email_operation(self, line):
        if '送信' in line or '送る' in line:
            return 'server.send_message(msg)'
        if '作成' in line or '準備' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                body = match.group(1)
                return f'MIMEText("{body}")'
        if '接続' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                server = match.group(1)
                return f'smtplib.SMTP("{server}")'
        return '# メール送信'
    
    # ========== スレッド処理 ==========
    
    def _is_threading_operation(self, line):
        return any(word in line for word in ['スレッド', '並列', 'thread', '並行', '非同期'])
    
    def _parse_threading_operation(self, line):
        if '開始' in line or '起動' in line:
            match = re.search(r'(.+?)を', line)
            if match:
                func = match.group(1)
                return f'threading.Thread(target={func}).start()'
        if '待機' in line or '終了待ち' in line:
            return 'thread.join()'
        return '# スレッド処理'
    
    # ========== タイマー ==========
    
    def _is_timer_operation(self, line):
        return any(word in line for word in ['タイマー', '定期', 'スケジュール', 'interval', '待機'])
    
    def _parse_timer_operation(self, line):
        if '秒待機' in line or '秒sleep' in line:
            match = re.search(r'(\d+)', line)
            if match:
                sec = match.group(1)
                return f'time.sleep({sec})'
        if '定期' in line:
            return '# 定期実行は threading.Timer を使用'
        return '# タイマー'
    
    # ========== サブプロセス ==========
    
    def _is_subprocess_operation(self, line):
        return any(word in line for word in ['コマンド実行', 'システム', 'シェル', 'subprocess', '外部コマンド'])
    
    def _parse_subprocess_operation(self, line):
        if '実行' in line:
            match = re.search(r'「(.+?)」', line)
            if match:
                cmd = match.group(1)
                return f'subprocess.run("{cmd}", shell=True, capture_output=True, text=True)'
        return '# サブプロセス'
